...
public class CSHZAction extends BaseAction {
	...
	if(score!=null && score.getId()!=0){
	    scoreService.retrial(score.getId());
	}
	return SUCCESS;
    }
    public String plfs(){
	scoreService.plfs(getRequest().getParameterValues("ids"));
	return SUCCESS;
    }
    public String pllq(){
	scoreService.pllq(getRequest().getParameterValues("ids"));
	return SUCCESS;
    }
    public String list() {
	...
	if (user != null && user.getSpecialty() != null) {
	   ...
	   if(key3!=null){
		if("1".equals(key3)){
		    hql.append(" and s.ifRetrial=1");
		}else if("2".equals(key3)){
		    hql.append(" and s.ifRetrial=0");
		}
	...
	hql
		.append(" order by s.specialtyId.id asc, (s.bjyw+s.bjsx+s.bjyy+s.hkzongfen+s.zonghecj+s.techangcj+s.cs) desc");
	...
	public InputStream getInputStream() throws Exception {
	   ...
	   hql.append(" order by s.specialtyId.id asc,(s.bjyw+s.bjsx+s.bjyy+s.hkzongfen+s.zonghecj+s.techangcj+s.cs) desc");
	...
	public HSSFWorkbook exportExcel(List dataList, String[] headers)
		...
		try {
			...
			HSSFSheet sheet = workbook.createSheet("CSHZ");
				...
				for (int i = 0; i < dataList.size(); i++) {
					...
					for (int j = 0; j < headers.length; j++) {
						if ("姓名".equals(headers[j])) {
							...
							this.createCell(row1, j, style2,HSSFCell.CELL_TYPE_STRING, o.getBjyw());
						}else if("数学".equals(headers[j])){
							this.createCell(row1, j, style2,HSSFCell.CELL_TYPE_STRING, o.getBjsx());
						}else if("英语".equals(headers[j])){
							this.createCell(row1, j, style2,HSSFCell.CELL_TYPE_STRING, o.getBjyy());
						...
						}else if("初试总分".equals(headers[j])){
							this.createCell(row1, j, style2,HSSFCell.CELL_TYPE_STRING, (o.getBjyw()+o.getBjsx()+o.getBjyy()+o.getHkzongfen()+o.getZonghecj()+o.getTechangcj()+o.getCs()));
						}else{}
					}
				}
			}
