...
<head>
	...
	<body class="middlebody">
		<form name="myform"  method="post" action="<%=request.getContextPath()%>/examinee/list.action">
			 <table align="center" class="table" border="1" width="100%"	style="border-collapse: collapse;">
				<tr>
					...
					<td class="content_L"  align="left" width="40%">
						<c:if test="${xy:hasPermission(user.id,'examinee',1)}">
							<input type="button" class="f_btn01" value="查询" onclick="submit()">
						</c:if>
					</td>
					<td class="content_L"  align="right" width="45%">
						<c:if test="${xy:hasPermission(user.id,'examinee',0)}">
							<input type="button" class="f_btn01" value="打印考场桌贴" onclick="openWin2('<%=path%>/examinee/zt.action','桌贴',800,760,1)">
						</c:if>
						&nbsp;
						<c:if test="${xy:hasPermission(user.id,'examinee',0)}">
							<input type="button" class="f_btn01" value="打印考场考生信息" onclick="openWin2('<%=path%>/examinee/ks.action','考生',800,750,1)">
						</c:if>
						<c:if test="${xy:hasPermission(user.id,'examinee',1)}">
	      	...
	      	<tr>
				...
				<td align="center" class="content_L">
					...
					<c:choose>
							...
							<a href="<%=path%>/examinee/toUpdate.action?o.id=${examinee.id }" target="_blank" class="bb">修改</a>
				...
				<c:choose>
					<c:when test="${empty examinee.admissionId}">
						<c:choose>
							...
							<a href="#" onclick="SC('<%=request.getContextPath() %>/examinee/sc.action?o.id=${examinee.id }','');">生成</a>
							</c:when>						
							<c:otherwise>
							...
							</c:otherwise>						
				...
				<c:choose>
					<c:when test="${not empty examinee.admissionId}">
							...
							<a href="#" onclick="openWin2('<%=request.getContextPath() %>/examinee/zkzPrint.action?o.id=${examinee.id }','准考证',600,440,1)">打印</a>
					</c:when>
					<c:otherwise>
					...
					</c:otherwise>
				...
				<td align="center" class="content_L">
					<c:choose>
						<c:when test="${xy:hasPermission(user.id,'examinee',3)}">
							<a href="#" onclick="del('<%=request.getContextPath() %>/examinee/del.action?o.id=${examinee.id }')">删除</a>
						</c:when>						
						<c:otherwise>
						...
						</c:otherwise>						
